           ...
           <?php 
            include 'conn.php';
             mysqli_set_charset( $con, 'utf8');
                $sql = "SELECT id, image, alt FROM slider_images";
                ...
                if (mysqli_num_rows($result) > 0) 
                    {
                        ...
                        {
                             echo '<tr><td>'.$i.'</td><td><img src="' . $row['image'] . '" alt="no image"height="150px" width="150px"/></td><td>'.$row["alt"].'</td></td><td><a href="slidedelete.php?id='.$row["id"].'">delete</a></td></tr>';
                                  $i++;
                        } 
                    }
                else
