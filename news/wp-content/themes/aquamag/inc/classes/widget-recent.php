 ...
 * @since      1.0.0
 */
class AquaMag_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-aquamag-recent posts-thumbnail-widget',
			'description' => __( 'Display recent posts with thumbnails.', 'aquamag' )
		);

		// Create the widget.
		$this->WP_Widget(
			'aquamag-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts Thumbnails', 'aquamag' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'aquamag_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'aquamag_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'aquamag_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'aquamag' ),
			'limit'     => 5,
			'show_date' => false
		);
