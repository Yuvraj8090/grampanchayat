 ...
 * @since      1.0.0
 */
class TrueNews_Ads125_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-truenews-ad125 ad-widget',
			'description' => __( 'Easily to display any type of ad.', 'truenews' )
		);

		// Create the widget.
		$this->WP_Widget(
			'truenews-ad125',                                  // $this->id_base
			__( '&raquo; Advertisement 125x125', 'truenews' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="add-section uk-clearfix">';
			...
			if ( $instance['ad_code1'] ) {
				echo '<div class="add-one add-img">' . stripslashes( $instance['ad_code1'] ) . '</div>';
			}

			if ( $instance['ad_code2'] ) {
				echo '<div class="add-img">' . stripslashes( $instance['ad_code2'] ) . '</div>';
			}

			if ( $instance['ad_code3'] ) {
				echo '<div class="add-one add-img">' . stripslashes( $instance['ad_code3'] ) . '</div>';
			}

			if ( $instance['ad_code4'] ) {
				echo '<div class="add-img">' . stripslashes( $instance['ad_code4'] ) . '</div>';
			}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'    => esc_html__( 'Advertisement', 'truenews' ),
			'ad_code1' => '',
			'ad_code2' => '',
			'ad_code3' => '',
			'ad_code4' => '',
