 ...
 * @since      1.0.0
 */
class TrueNews_Feedburner_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-truenews-feedburner subscribe-widget',
			'description' => __( 'FeedBurner email subscription.', 'truenews' )
		);

		// Create the widget.
		$this->WP_Widget(
			'truenews-feedburner',                  // $this->id_base
			__( '&raquo; Feedburner', 'truenews' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( $instance['feed_id'] ) { ?>
			<form class="form-subscribe" action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo strip_tags( $instance['feed_id'] ); ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520'); return true">
				...
				<?php } ?>
				<input type="text" name="email" placeholder="<?php esc_attr_e( 'Enter your email', 'truenews' ); ?>">
				...
				<button class="btn" type="submit" name="submit"><?php _e( 'Subscribe', 'truenews' ); ?></button>
			</form>
		<?php 
		}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'   => esc_html__( 'Get Updates', 'truenews' ),
			'before'  => esc_html__( 'Subscribe to our newsletter to receive breaking news by email.', 'truenews' ),
			'feed_id' => ''
		);
	...
	?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'before' ); ?>">
				<?php _e( 'HTML or Text before form:', 'truenews' ); ?>
			</label>
